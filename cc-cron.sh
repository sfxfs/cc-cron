...
require_job_id() {
    local command="$1"; shift; [[ $# -lt 1 ]] && error "Usage: cc-cron ${command} <job-id>" "$EXIT_INVALID_ARGS" || true
}
...
crontab_add_entry() {
    local entry="$1"; (crontab -l 2>/dev/null; echo "$entry") | crontab -; invalidate_crontab_cache
}
...
generate_run_script() {
    ...
    local claude_opts="-p"; [[ -n "$job_model" ]] && claude_opts="$claude_opts --model $job_model"; [[ "$job_permission" != "default" ]] && claude_opts="$claude_opts --permission-mode $job_permission"
...
cmd_pause() {
    local job_id="$1" paused_file meta_file; paused_file="${DATA_DIR}/${job_id}.paused"; meta_file=$(get_meta_file "$job_id")
...
# Resume a paused job
cmd_resume() {
    local job_id="$1" paused_file; paused_file="${DATA_DIR}/${job_id}.paused"
...
cmd_show() {
    ...
    local log_file; log_file=$(get_log_file "$job_id"); [[ -f "$log_file" ]] && echo "  Log file: ${log_file}" || true
}
...
cmd_status() {
    info "CC-Cron Status Report"; echo -e "======================\n"
    ...
    for meta_file in "${LOG_DIR}"/*.meta; do
        ...
        if [[ -f "$status_file" ]]; then
            ...
            local last_run; last_run=$(get_stat "$log_file" mtime | cut -d. -f1); echo -e "  ${id}: ${YELLOW}? NO STATUS${NC} (last activity: ${last_run})\n    Workdir: ${workdir}\n"; ((unknown_count++)) || true
        fi
    done
...
_show_job_stats() {
    ...
    if [[ -f "$history_file" ]]; then
        while IFS= read -r line; do
            ...
            total_duration=$((total_duration + end_ts - start_ts)); ((duration_count++)) || true
        done < "$history_file"
    fi
...
cmd_import() {
    ...
    # Parse JSON
    local job_count; job_count=$(jq '.jobs | length' "$input_file"); [[ "$job_count" -eq 0 ]] && { warn "No jobs found in import file"; return 0; }; info "Found ${job_count} job(s) to import"
...
cmd_purge() {
    ...
    for run_script in "${DATA_DIR}"/run-*.sh; do
        [[ -f "$run_script" ]] || continue; local job_id; job_id=$(basename "$run_script" .sh); job_id="${job_id#run-}"
    ...
    echo; [[ "$dry_run" == "true" ]] && info "Dry-run summary:" || success "Purge complete:"; echo -e "  Logs purged:     ${purged_logs}\n  History purged:  ${purged_history}\n  Orphans removed: ${purged_orphans}\n  Space freed:     ${freed_mb} MB"
}
...
cmd_config() {
    ...
    case "$action" in
        list)
            ...
            [[ -f "$CONFIG_FILE" ]] && { echo -e "Config file contents:\n----------------------"; cat "$CONFIG_FILE"; } || echo -e "No config file exists. Create one with:\n  cc-cron config set workdir /path/to/dir\n  cc-cron config set model sonnet"
            ;;
        set)
            local key="${2:-}" value="${3:-}"
...
cmd_doctor() {
    ...
    while IFS= read -r line; do
        [[ "$line" == *"${CRON_COMMENT_PREFIX}"* ]] || continue
        ((crontab_jobs++)) || true; local job_id meta_file; job_id=$(extract_job_id "$line"); meta_file=$(get_meta_file "$job_id")
        [[ -f "$meta_file" ]] || { echo "   ! Missing metadata for job: ${job_id}"; ((orphaned++)) || true; }
    done < <(get_crontab)

    # Count meta files
    for meta_file in "${LOG_DIR}"/*.meta; do [[ -f "$meta_file" ]] || continue; ((meta_files++)) || true; done; echo -e "   Jobs in crontab: ${crontab_jobs}\n   Metadata files:  ${meta_files}"
    ...
    # Summary
    echo -e "\n================================"
    [[ $issues -eq 0 && $warnings -eq 0 ]] && echo -e "${GREEN}All checks passed!${NC}\n" || { [[ $issues -gt 0 ]] && echo -e "${RED}Found ${issues} issue(s) that need attention${NC}"; [[ $warnings -gt 0 ]] && echo -e "${YELLOW}Found ${warnings} warning(s)${NC}\n"; }
...
cmd_help() {
    ...
    case "$topic" in
        ...
        "")
            ...
            ;;
        *)
            echo -e "Unknown help topic: $topic\nRun 'cc-cron help' for available commands"; return 1
            ;;
    esac
