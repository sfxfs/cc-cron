...
extract_job_id() {
	local line="$1"
	echo "${line#*"${CRON_COMMENT_PREFIX}"}" | cut -d: -f1
}
...
validate_cron_field() {
	...
	*,*)
		local IFS=',' part
		for part in $value; do validate_cron_field "$part" "$min" "$max" "$field_name"; done
		return 0
		;;
	esac
...
write_meta_file() {
	...
	local safe_prompt safe_workdir safe_model safe_permission safe_run_script
	safe_prompt=$(escape_shell_string "$prompt")
	safe_workdir=$(escape_shell_string "$workdir")
	safe_model=$(escape_shell_string "$model")
	safe_permission=$(escape_shell_string "$permission")
	safe_run_script=$(escape_shell_string "$run_script")
		...
		if [[ "$line" == *"${CRON_COMMENT_PREFIX}"* ]]; then
			...
			if [[ -f "$meta_file" ]]; then
				...
				if [[ "$json_output" == "true" ]]; then
					...
					local escaped_prompt escaped_workdir escaped_permission
					escaped_prompt=$(escape_json_string "$prompt")
					escaped_workdir=$(escape_json_string "${workdir:-$CC_WORKDIR}")
					escaped_permission=$(escape_json_string "${permission_mode:-$CC_PERMISSION_MODE}")
					local job_json="{"
					job_json+="\"id\":\"${id}\""
...
# Show logs for a job
cmd_logs() {
	local job_id="$1" follow="${2:-false}" log_file
	log_file=$(get_log_file "$job_id")
...
cmd_pause() {
	...
	paused_file="${DATA_DIR}/${job_id}.paused"
	meta_file=$(get_meta_file "$job_id")
...
calculate_next_run() {
	...
	if [[ "$minute" == "*" && "$hour" == "*" ]]; then
		...
		if [[ "$minute" == */* ]]; then
			...
			if [[ "$step" =~ ^[0-9]+$ ]]; then
				# Every N minutes
				local current_minute minutes_until
				current_minute=$(date +%M)
				current_minute=$((10#$current_minute))
				minutes_until=$(((step - current_minute % step) % step))
			...
			local current_minute target_minute minutes_until
			current_minute=$(date +%M)
			current_minute=$((10#$current_minute))
			target_minute=$((10#$minute))
			minutes_until=$(((target_minute - current_minute + 60) % 60))
		...
		if [[ "$hour" == */* ]]; then
			...
			if [[ "$hour_step" =~ ^[0-9]+$ ]]; then
				...
				local current_hour current_minute target_minute hours_until
				current_hour=$(date +%H)
				current_minute=$(date +%M)
				current_hour=$((10#$current_hour))
				current_minute=$((10#$current_minute))
				target_minute=$((10#$minute))
				hours_until=$(((hour_step - current_hour % hour_step) % hour_step))
			...
			local current_hour current_minute target_hour target_minute minutes_today minutes_now minutes_until
			current_hour=$(date +%H)
			current_minute=$(date +%M)
			current_hour=$((10#$current_hour))
			current_minute=$((10#$current_minute))
			target_hour=$((10#$hour))
			target_minute=$((10#$minute))
			minutes_today=$((target_hour * 60 + target_minute))
			minutes_now=$((current_hour * 60 + current_minute))
			minutes_until=$((minutes_today - minutes_now))
		...
		if [[ "$weekday" =~ ^[0-6]$ ]]; then
			...
			local current_weekday target_weekday
			current_weekday=$(($(date +%u) % 7))
			target_weekday=$((10#$weekday)) # 0-6, Sunday is 0

			local current_hour current_minute target_hour target_minute days_until
			current_hour=$(date +%H)
			current_minute=$(date +%M)
			current_hour=$((10#$current_hour))
			current_minute=$((10#$current_minute))
...
cmd_show() {
	...
	if [[ -f "$history_file" ]]; then
		...
		read -r total_runs success_runs failed_runs < <(awk '/status=success/{s++} /status=failed/{f++} END{print NR, s+0, f+0}' "$history_file")
...
cmd_edit() {
	...
	local timestamp new_run_script
	timestamp=$(date '+%Y-%m-%d %H:%M:%S')
	new_run_script=$(get_run_script "$job_id")
...
cmd_status() {
	...
	for meta_file in "${LOG_DIR}"/*.meta; do
		...
		status_file=$(get_status_file "$id")
...
_show_job_stats() {
	...
	if [[ -f "$history_file" ]]; then
		while IFS= read -r line; do
			...
			total_duration=$((total_duration + end_ts - start_ts))
			((duration_count++)) || true
		done <"$history_file"
	fi
	...
	[[ $total_runs -gt 0 ]] && echo "  Success rate: $((success_count * 100 / total_runs))%" || true
	[[ $duration_count -gt 0 ]] && echo "  Avg duration: $(((total_duration / duration_count) / 60))m $(((total_duration / duration_count) % 60))s" || true
...
cmd_export() {
	...
	for job_id in "${jobs[@]}"; do
		...
		# Check if paused
		local paused_file="${DATA_DIR}/${job_id}.paused" is_paused
		is_paused="$([[ -f "$paused_file" ]] && echo true || echo false)"
		...
		escaped_prompt=$(escape_json_string "$prompt")
		escaped_workdir=$(escape_json_string "$workdir")
		escaped_model=$(escape_json_string "${model:-}")
		escaped_permission=$(escape_json_string "$permission_mode")
		escaped_tags=$(escape_json_string "${tags:-}")
...
cmd_import() {
	...
	for ((i = 0; i < job_count; i++)); do
		...
		eval "$(jq -r --argjson i "$i" '.jobs[$i] | "job_cron=\(.cron|@sh) job_prompt=\(.prompt|@sh) job_recurring=\(.recurring) job_workdir=\(.workdir|@sh) job_model=\(.model|@sh) job_permission=\(.permission_mode|@sh) job_timeout=\(.timeout) job_paused=\(.paused) job_tags=\(.tags|@sh)"' "$input_file")" 2>/dev/null || {
			warn "Skipping malformed job at index ${i}"
			((skipped++)) || true
			continue
		}
...
purge_old_files() {
	...
	for file in "${dir}"/*.${ext}; do
		...
		file_mtime=$(get_stat "$file" mtime_unix 2>/dev/null) || continue
...
cmd_doctor() {
	...
	local data_size available_space
...
_cc_cron_completion() {
    ...
    _get_job_ids() {
        local meta_file; for meta_file in ~/.cc-cron/logs/*.meta; do
            [[ -f "$meta_file" ]] || continue; basename "$meta_file" .meta
        done
    }

    _get_tags() {
        local meta_file tags; for meta_file in ~/.cc-cron/logs/*.meta; do
...
Options:
  ...
  --quiet, -q                 Only output the job ID (useful for scripting)" "$EXIT_INVALID_ARGS"
		...
		local job_timeout quiet="false" job_tags=""
	...
	purge)
		...
		while [[ $# -gt 0 ]]; do case "$1" in --dry-run)
			dry_run="true"
			shift
			;;
		*)
